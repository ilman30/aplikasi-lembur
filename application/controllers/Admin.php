...
class Admin extends CI_Controller
{
	...
	function tambah_lembur(){
		$data['staff'] = $this->modcrud->data_staff();
		$this->load->view('admin/datalembur_tambah', $data);
	}

	function aksi_tambah_lembur(){
		$nik = $this->input->post('nik_staff', TRUE);
		$staff = $this->modcrud->data_staff_byId($nik);
		if($staff == null){
			$this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissible alert-custom">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					<h4><i class="icon fa fa-ban"></i> Failed!</h4> Staff Tidak Ada
					</div>');
			redirect('tambah-lembur', 'refresh');
		}else{
			$data = array(
				'nik_staff' => $this->input->post('nik_staff', TRUE),
				'tunj_lembur' => $this->input->post('tunj_lembur', TRUE),
				'jumlah_jam' => $this->input->post('jumlah_jam', TRUE),
				'total_tunj_lembur' => $this->input->post('total_tunj_lembur', TRUE)
			);
			$this->modcrud->input_dosen('lembur', $data);
			redirect('data-lembur', 'refresh');
