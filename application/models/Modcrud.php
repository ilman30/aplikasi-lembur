...
class Modcrud extends CI_Model
{
	...
	//data-staff
	function data_staff(){
		$sql = $this->db->query("SELECT * FROM staff")->result();
		return $sql;
	}

	function data_staff_byId($nik){
		$sql = $this->db->query("SELECT * FROM staff WHERE nik_staff = '".$nik."' ")->result();
		return $sql;
	}

	//lembur
	function data_lembur(){
		$sql = $this->db->query("SELECT a.nik_staff, a.tunj_lembur, a.jumlah_jam, a.total_tunj_lembur, b.nama_staff 
								 FROM lembur a JOIN staff b ON b.nik_staff = a.nik_staff")->result();
		return $sql;
	}

	function input_lembur($table, $data)
	{
